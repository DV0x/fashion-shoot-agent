...
Based on user's reference images, you can customize:
- `{BACKGROUND}` - Environment/setting (e.g., "studio grey seamless", "industrial warehouse", "outdoor urban street")
- `{WARDROBE}` - Clothing description (e.g., "black leather jacket, white t-shirt, dark jeans")

If not specified, analyze and preserve from the input image.
...
│  R2C1         │  R2C2         │  R2C3         │
│  Side-On      │  Close        │  Extreme      │
│  Profile      │  Portrait     │  Detail       │
└───────────────┴───────────────┴───────────────┘
```
...
(e.g., "moody industrial warehouse with concrete floors and exposed pipes")
```

**Change wardrobe:**
```
Wardrobe: {WARDROBE}
(e.g., "oversized camel coat, black turtleneck, tailored grey trousers, white sneakers")
```
